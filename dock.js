...
function buildDock() {
    ...
    DOCK_ICONS.forEach((icon) => {
        ...
        wrap.addEventListener("click", () => {
            ...
            if (icon.id === "trash"     && typeof window.openTrashWindow     === "function") window.openTrashWindow();
            if (icon.id === "notes"     && typeof window.openNotesWindow     === "function") window.openNotesWindow();
...
(function () {
    ...
    function applyBlobUrl(blobUrl) {
        wl.style.cssText = `position:absolute;inset:0;background-image:url(${blobUrl});background-size:cover;background-position:center;background-repeat:no-repeat;z-index:0;`;
    }
    function applyGradient() {
        wl.style.cssText = `position:absolute;inset:0;background:linear-gradient(155deg,#061224 0%,#0b2545 20%,#0e3d6e 40%,#1a5a8a 58%,#2c7bb0 72%,#4da0c8 84%,#89c8d8 93%,#d0ecd8 100%);z-index:0;`;
    }
