...
interface WindowConfig {
  ...
  width:   number;
  height:  number;
  content: (win: { el: HTMLElement; close: () => void }) => HTMLElement;
}
...
// ── z-index tracker ──
let _activeZ = 10;
const _wins  = new Map<string, WinState>();
let _activeId: string | null = null;

function _raise(state: WinState): void {
  _activeZ += 2;
...
function _setActive(appId: string): void {
  _wins.forEach((s, id) => {
    s.el.classList.toggle("active", id === appId);
    const tl = s.el.querySelector<HTMLElement>(".tl-lights");
    if (tl) tl.dataset.focused = id === appId ? "true" : "false";
    const tlContainer = s.el.querySelector<HTMLElement>(".tl-container");
    if (tlContainer) tlContainer.classList.toggle("unfocused", id !== appId);
  });
  _activeId = appId;
...
// ── Drag ──
function _makeDraggable(el: HTMLElement, handle: HTMLElement, appId: string): void {
...
// ── Maximize ──
function _maximize(el: HTMLElement, appId: string, w: number, h: number): void {
...
// ── Main: createWindow ──
function createWindow(cfg: WindowConfig): void {
  ...
  if (!area) { console.error("No #windows-area found"); return; }
  ...
  function closeApp(): void {
    ...
    if (m) win.style.transform = `translate(${m[1]}px, ${m[2]}px) scale(0.95)`;
