...
class PhotosGrid {
    constructor(cbs) {
        ...
        this.zoomLevel = 3; // 1=small … 5=large
    ...
    setViewMode(mode) { this.viewMode = mode; this._render(); }
    setFilter(filter) { this.filter = filter; this._render(); }
    setZoom(level) { this.zoomLevel = Math.max(1, Math.min(5, level)); this._render(); }
    refresh() { this._render(); }
    showSearchResults(photos) {
        this.el.innerHTML = "";
        if (!photos.length) {
    ...
    _getPhotos() {
        ...
        switch (this.filter) {
            case "favorites":
                photos = this.store.getFavorites();
                break;
            case "videos":
                photos = this.store.getByMediaType("video");
                break;
            case "selfies":
                photos = this.store.getByMediaType("selfie");
                break;
            case "portraits":
                photos = this.store.getByMediaType("portrait");
                break;
            case "panoramas":
                photos = this.store.getByMediaType("panorama");
                break;
            case "livePhotos":
                photos = this.store.getByMediaType("livePhoto");
                break;
            case "bursts":
                photos = this.store.getByMediaType("burst");
                break;
        }
        return photos;
    }
    ...
    _renderMonths(photos) {
        const groups = this.store.groupByMonth(photos);
        groups.forEach((groupPhotos, label) => {
            const group = document.createElement("div");
    ...
    _renderYears(photos) {
        ...
        groups.forEach((groupPhotos, year) => {
            const group = document.createElement("div");
            ...
            groupPhotos.slice(0, 12).forEach((photo) => {
    ...
    _makePhotoCell(photo, size) {
        ...
        cell.addEventListener("click", (e) => {
            if (e.target.closest(".ph-fav-btn, .ph-sel-circle"))
                return;
    ...
    _toggleSelect(id) {
        ...
        this.el.querySelectorAll(`[data-id="${id}"]`).forEach(el => {
            ...
            const circle = el.querySelector(".ph-sel-circle");
            if (circle)
                circle.classList.toggle("checked", this.selected.has(id));
        });
        this.cbs.onPhotoSelect(this.selected);
    }
    _showContextMenu(e, photo) {
...
window.__PhotosGrid = PhotosGrid;
