...
class PhotosApp {
  ...
  private viewMode: "all" | "months" | "years" = "all";
  ...
  private _build(): void {
    // ── Sidebar ──
    this.sidebar = new (window as any).__PhotosSidebar({
      onSection: (section: any) => this._handleSection(section),
    });
    ...
    // Grid
    this.currentPhotos = this.store.getAll();
    this.grid = new (window as any).__PhotosGrid({
      onPhotoOpen:      (id: number) => this._openLightbox(id),
      onFavoriteToggle: (id: number) => this.store.toggleFavorite(id),
      onSetWallpaper:   (src: string) => this._setWallpaper(src),
      onPhotoSelect:    (ids: Set<number>) => {
    ...
    // ── Lightbox ──
    this.lightbox = new (window as any).__PhotosLightbox({
      onClose:        () => {},
      onFavorite:     (id: number) => this.store.toggleFavorite(id),
      onSetWallpaper: (src: string) => this._setWallpaper(src),
    });
  ...
  private _buildToolbar(): HTMLElement {
    ...
    requestAnimationFrame(() => {
      ...
      document.getElementById("phZoomSlider")?.addEventListener("input", (e) => {
        this.grid.setZoom(+(e.target as HTMLInputElement).value);
      });
      document.getElementById("phZoomIn") ?.addEventListener("click", () => {
        const sl = document.getElementById("phZoomSlider") as HTMLInputElement;
      ...
      // View tabs
      tb.querySelectorAll<HTMLElement>(".ph-view-tab").forEach(tab => {
        tab.addEventListener("click", () => {
          tb.querySelectorAll(".ph-view-tab").forEach(t => t.classList.remove("active"));
          tab.classList.add("active");
          this.viewMode = tab.dataset.mode as any;
      ...
      document.getElementById("phFilterBtn")?.addEventListener("click", (e) => {
        e.stopPropagation();
        this._showFilterMenu(e as MouseEvent);
      });

      // Search
      const searchInput = document.getElementById("phSearch") as HTMLInputElement;
  ...
  private _showFilterMenu(e: MouseEvent): void {
    ...
    filters.forEach(f => {
      ...
      item.addEventListener("click", () => {
        ...
        this.grid.setFilter(f.value as any);
  ...
  private _handleSection(section: any): void {
    ...
    switch (section) {
      ...
      case "videos":     this.grid.setFilter("videos");     break;
      case "selfies":    this.grid.setFilter("selfies");    break;
      case "portraits":  this.grid.setFilter("portraits");  break;
      case "panoramas":  this.grid.setFilter("panoramas");  break;
      case "livePhotos": this.grid.setFilter("livePhotos"); break;
      case "bursts":     this.grid.setFilter("bursts");     break;
      default:
        this.currentPhotos = this.store.getAll();
        this.grid.setFilter("all");
        break;
  ...
  private _openLightbox(id: number): void {
    const photos = this.store.getAll();
    this.lightbox.open(id, photos);
  }

  private _setWallpaper(src: string): void {
  ...
  private _updateStatus(selectedCount: number, override?: string): void {
...
// ── Register globally ──
(window as any).openPhotosWindow = function(): void {
  if (typeof (window as any).__createWindow !== "function") return;
