...
interface DockIcon {
  id: string; label: string; src: string; emoji: string;
  running: boolean; separator?: boolean;
}
...
interface SpringState { value: number; target: number; velocity: number; }
interface IconState   { wrap: HTMLElement; imgEl: HTMLElement; spring: SpringState; raf: number; }
...
function piecewiseInterp(x: number): number {
  if (x <= DIST_INPUT[0]) return WIDTH_OUT[0];
  if (x >= DIST_INPUT[DIST_INPUT.length-1]) return WIDTH_OUT[WIDTH_OUT.length-1];
...
function animateIcon(state: IconState): void {
  ...
  if (!settled) state.raf = requestAnimationFrame(() => animateIcon(state));
}
...
function buildDock(): void {
  ...
  DOCK_ICONS.forEach((icon) => {
    ...
    const state: IconState = { wrap, imgEl: img, spring: { value: BASE_WIDTH, target: BASE_WIDTH, velocity: 0 }, raf: 0 };
    ...
    wrap.addEventListener("click", () => {
      ...
      if (icon.id === "trash" && typeof (window as any).openTrashWindow === "function") {
        (window as any).openTrashWindow();
      }
      if (icon.id === "notes" && typeof (window as any).openNotesWindow === "function") {
        (window as any).openNotesWindow();
      }
      if (icon.id === "finder" && typeof (window as any).openFinderWindow === "function") {
...
dock.addEventListener("mouseenter", (e: MouseEvent) => { mouseX = e.clientX; scheduleAll(); });
dock.addEventListener("mousemove",  (e: MouseEvent) => { mouseX = e.clientX; scheduleAll(); });
...
(function() {
  ...
  function applyBlobUrl(blobUrl: string): void {
    wl.style.cssText = `
