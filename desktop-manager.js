...
class DesktopManager {
    constructor() {
        ...
        this.clipboard = { ids: [], cut: false };
        // Rubber band
        this.rb = { active: false, sx: 0, sy: 0 };
        // Drag state
        this.drag = {
            active: false,
            nodeId: "",
    ...
    _startIconDrag(e, nodeId, el) {
        ...
        const onMove = (e) => {
            ...
            this.selected.forEach(id => {
                const iconEl = this.iconsEl.querySelector(`[data-node-id="${id}"]`);
                if (!iconEl)
                    return;
    ...
    // ─────────────────────────────────────────────
    _bindDesktopEvents() {
        this.iconsEl.addEventListener("mousedown", (e) => {
            ...
            const onMove = (e) => {
                ...
                this.iconsEl.querySelectorAll(".dt-icon").forEach(el => {
        ...
        this.iconsEl.addEventListener("dragover", (e) => {
            e.preventDefault();
        ...
        this.iconsEl.addEventListener("drop", (e) => {
            e.preventDefault();
    ...
    _bindKeyboard() {
        document.addEventListener("keydown", (e) => {
            ...
            if (meta && e.key === "a") {
                e.preventDefault();
                this.fs.getChildren("desktop").forEach((n) => this.selected.add(n.id));
                this._updateSelectionVisual();
            }
            if (meta && e.key === "c") {
                e.preventDefault();
            ...
            if (meta && e.key === "x") {
                ...
                // Dim cut icons
                this.selected.forEach(id => {
                    this.iconsEl.querySelector(`[data-node-id="${id}"]`)?.classList.add("dt-cut");
                });
    ...
    _paste() {
        ...
        this.clipboard.ids.forEach((id, i) => {
            ...
            if (this.clipboard.cut) {
                ...
                this.iconsEl.querySelector(`[data-node-id="${id}"]`)?.classList.remove("dt-cut");
            }
            else {
                // Duplicate
    ...
    `;
        ...
        menu.querySelector("#dtcSelAll")?.addEventListener("click", () => { menu.remove(); this.fs.getChildren("desktop").forEach((n) => this.selected.add(n.id)); this._updateSelectionVisual(); });
    ...
    _importFile(file, x, y) {
        const pos = this._clamp(x, y);
        if (file.type.startsWith("image/")) {
            const reader = new FileReader();
            reader.onload = () => {
                const node = this.fs.createImageFile("desktop", file.name, reader.result, file.size);
            ...
            const reader = new FileReader();
            reader.onload = () => {
                const node = this.fs.createTextFile("desktop", file.name, reader.result);
                this.positions.set(node.id, pos);
                this._savePositions();
            };
    ...
    _openNode(node) {
        if (node.type === "folder") {
            ...
            if (typeof window.openFinderWindow === "function") {
                window.__finderNavigateTo = node.id;
            ...
            if (typeof window.openNotesWindow === "function") {
                ...
                let existing = store?.getNotes().find((n) => n.title === node.name);
                if (!existing && store) {
    ...
    _startRename(nodeId) {
        const iconEl = this.iconsEl.querySelector(`[data-node-id="${nodeId}"]`);
    ...
    _getDropTarget(cx, cy) {
        const wins = document.querySelectorAll(".app-window[data-app-id='finder']");
        for (const win of Array.from(wins)) {
            const r = win.getBoundingClientRect();
            if (cx >= r.left && cx <= r.right && cy >= r.top && cy <= r.bottom) {
                return window.__finderCurrentFolder ?? null;
            }
        }
        return null;
    }
    _highlightDropTargets(cx, cy) {
        document.querySelectorAll(".app-window[data-app-id='finder']").forEach(win => {
            const r = win.getBoundingClientRect();
    ...
    _loadPositions() {
        try {
            ...
            if (raw) {
                ...
                Object.entries(obj).forEach(([id, pos]) => this.positions.set(id, pos));
            }
        }
        catch { }
    }
...
window.__initDesktopManager = function () {
    new DesktopManager();
};
