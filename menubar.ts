...
interface MenuItem {
  label?: string; shortcut?: string; disabled?: boolean; sep?: boolean;
}
...
function buildActionCenter(): void {
  ...
  document.getElementById("acPlay")?.addEventListener("click", () => {
    ...
    if (icon) icon.innerHTML = MB.nowPlaying.playing
      ? `<path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>`
      : `<path d="M8 5v14l11-7z"/>`;
  });

  // Screenshot
  document.querySelector<HTMLElement>('[title="Screenshot"]')?.addEventListener("click", () => {
