...
class LockScreen {
    ...
    show() {
        ...
        if (dock) {
            dock.style.transition = "transform 0.4s ease";
    ...
    `;
        ...
        const video = this.el.querySelector("#lsVideo");
        const fallback = this.el.querySelector("#lsVideoFallback");
    ...
    _updateClock() {
        ...
        const dateEl = this.el.querySelector("#lsDate");
        const timeEl = this.el.querySelector("#lsTime");
        if (dateEl)
    ...
    _bindEvents() {
        ...
        this.el.querySelector("#lsUserWrap")?.addEventListener("click", () => {
            const btn = this.el.querySelector("#lsEnterBtn");
            if (btn)
                btn.classList.add("visible");
            const hint = this.el.querySelector("#lsHint");
            if (hint)
    ...
    // ─────────────────────────────────────────────
    _flashDots() {
        const dots = this.el.querySelectorAll(".ls-dot");
        dots.forEach((dot, i) => {
...
// ── Singleton ──
let _lockScreenInstance = null;
function getLockScreen() {
    if (!_lockScreenInstance)
        _lockScreenInstance = new LockScreen();
